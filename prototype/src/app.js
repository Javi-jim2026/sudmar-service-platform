...
const filterLabels={q:'Búsqueda',folio:'Folio',priority:'SLA técnico',client:'Cliente',businessUnit:'Unidad de negocio',area:'Área',equipmentType:'Tipo de equipo',owner:'Responsable del ticket',status:'Estado',model:'Modelo',serial:'Serie',from:'Desde',to:'Hasta',overdue:'Fuera de plazo',activeOnly:'Tickets activos'};
const statusStyle={
...
const activityStatusTone=task=>{
 ...
 if((task.isCancelled||['cancelada','cancelado'].includes(normalized(task.status)))) return 'slate';
 if(task.dueAt&&task.dueAt<today) return 'red';
 if(normalized(task.status)==='bloqueada') return 'red';
 if(['en proceso','en ejecucion'].includes(normalized(task.status))) return 'purple';
...
const currentTasks=()=>tasksInScope().filter(t=>{
 ...
 const statusMatch=state.taskStatus==='all'
   ...
   ||(state.taskStatus==='cancelled'&&(t.isCancelled||['cancelada','cancelado'].includes(normalized(t.status))));
 let typeMatch=true;
...
function calendarActivityTasks(tickets){
 ...
 if(state.calendarActivityStatus==='cancelled') tasks=tasks.filter(t=>(t.isCancelled||['cancelada','cancelado'].includes(normalized(t.status))));
 return tasks;
}
...
function activityCalendarClass(task){
 ...
 if((task.isCancelled||['cancelada','cancelado'].includes(normalized(task.status)))) return 'cancelled';
...
function renderTeam(tickets){
 ...
 const cards=people.map(person=>{const w=personWorkload(person,tickets,tasks);return `<button class="person-card" data-action="team-person" data-person="${e(person.name)}"><span class="person-card-top"><span class="avatar large">${e(person.name.split(/\s+/).slice(0,2).map(x=>x[0]).join(''))}</span><span class="number-badge">${n(w.pending.length)} pendientes</span></span><h2>${e(person.name)}</h2><h3>${e(person.title)}</h3><p>${e((person.operationalAreas||[person.area]).join(' · '))}</p><div class="person-tags">${person.fieldTechnician?'<span class="badge blue">Técnico de campo</span>':''}${person.fieldLead?'<span class="badge green">Responsable de campo</span>':''}</div><span class="entity-card-foot"><span><strong>${n(w.ownedTickets.length)}</strong> tickets</span><span><strong>${n(w.assignedTasks.length)}</strong> actividades</span><span>${n(w.overdue.length)} vencidas</span></span></button>`;}).join('');
...
function checklistEditor(id,category,items=[]){
 return `<section id="${id}" class="work-editor full-width" ><h3>Lista de trabajos ${help("Trabajos internos de esta actividad. El avance no cambia automáticamente su estado.")}</h3><div class="work-items">${items.map(workItemRow).join('')}</div><p class="work-summary" aria-live="polite"></p><button type="button" class="button secondary small" data-action="add-work">+ Agregar trabajo</button></section>`;
}
...
async function submitNewTask(event){
 ...
 if(!repository.canWrite()){error.textContent='La conexión de escritura con Supabase todavía no está disponible.';return;}
 let payload;try{payload={...readActivityFields('newTask'),ticketFolio:$('newTaskTicket').value};}catch(err){error.textContent=err.message;return;}
...
document.addEventListener('click',event=>{const b=event.target.closest('[data-action]');if(!b)return;const a=b.dataset.action;if(!state.data&&!['close-dialog'].includes(a))return;
 ...
 if(a==='save-task-update'){(async()=>{
  ...
  try{
    ...
    const changes=readActivityFields('taskEdit',task);
    validateActivity(changes);
    if(changes.taskType===task.taskType)delete changes.title;
 ...
 if(a==='quick'){state.filters.status=b.dataset.key==='wait'?'EN ESPERA':b.dataset.key==='validation'?'PENDIENTE DE VALIDACIÓN':'';state.filters.activeOnly=b.dataset.key==='active';state.filters.overdue=false;state.page=1;render();}
...
const catalogs=()=>state.data.catalogs||{clients:[],models:[],serials:[],categories:[],types:[],statuses:[]};
const help=(text)=>`<details class="context-help"><summary aria-label="Consultar ayuda">ⓘ</summary><span>${e(text)}</span></details>`;
...
function ticketFields(prefix,t={}){
 ...
 `<div class="full-width"><p id="${prefix}summary" class="note-text" aria-live="polite">${e(t.description||'El resumen se generará con los cuatro campos.')}</p></div>`;
}
function readTicketFields(prefix,original={}){
 const val=k=>clean($(prefix+k)?.value),c=catalogs();
 const client=c.clients.find(x=>catalogKey(x.name)===catalogKey(val('client')));if(!client)throw Error('Selecciona un cliente existente o agrega uno nuevo.');
 const businessUnit=val('businessUnit');if(!['SUDMAR','PRETTL'].includes(businessUnit)&&businessUnit!==original.businessUnit)throw Error('Selecciona SUDMAR o PRETTL.');
 const category=c.categories.find(x=>x.business_unit===businessUnit&&catalogKey(x.name)===catalogKey(val('serviceCategory')));
 if(['SUDMAR','PRETTL'].includes(businessUnit)&&!category)throw Error('Selecciona o agrega una categoría para esta unidad.');
 const model=c.models.find(x=>catalogKey(x.name)===catalogKey(val('model'))),serial=c.serials.find(x=>x.model_id===model?.id&&catalogKey(x.serial_number)===catalogKey(val('serial')));
 if(val('model')&&!model&&val('model')!==original.model)throw Error('Selecciona o agrega el modelo al catálogo.');
 if(val('serial')&&!serial&&!(val('serial')===original.serial&&val('model')===original.model))throw Error('Selecciona o agrega una serie de este modelo.');
 const requestContext=Object.fromEntries(Object.keys(requestQuestions).map(k=>[k,val(k)]));
 const hasRequest=Object.values(requestContext).some(Boolean);
...
function activityFields(prefix,task={}){
 ...
 catalogControl(prefix,'antecedentFolio','Ticket antecedente / reincidencia',state.data.tickets.filter(t=>t.folio!==task.ticketFolio).map(t=>`${t.folio} · ${t.client} · ${t.model} · ${t.serial}`),task.antecedentFolio||'')+help('Opcional. Otra atención previa relacionada. Busca por folio, cliente, modelo o serie.')+
 field('Fecha de inicio',`<input type="date" id="${prefix}startAt" name="startAt" value="${e(task.startAt||today)}" required>`)+
 field('Fecha objetivo / término',`<input type="date" id="${prefix}dueAt" name="dueAt" value="${e(task.dueAt||'')}">`)+
 `<div class="full-width">${checklistEditor(prefix+'Checklist','OPERACIONES',task.checklist||[])}</div>`+
 field('Observaciones / requerimiento',`<textarea id="${prefix}notes" name="notes" rows="3" maxlength="5000">${e(task.notes||'')}</textarea>`)+
 field('Resultado / condición final',`<input id="${prefix}outcome" name="outcome" value="${e(task.outcome||'')}" maxlength="500">`,'Registra aquí reprogramación, no realizada, técnico no acudió u otra incidencia.')+
 field('Resolución',`<textarea id="${prefix}resolution" name="resolution" rows="3" maxlength="5000">${e(task.resolution||'')}</textarea>`,'Qué ocurrió realmente al ejecutar o cerrar la actividad.');
}
function readActivityFields(prefix,task={}){
 const v=k=>clean($(prefix+k)?.value),type=catalogs().types.find(t=>catalogKey(t.name)===catalogKey(v('taskType')));
 if(!type&&v('taskType')!==task.taskType)throw Error('Selecciona o agrega un tipo de actividad.');
 if(!v('owner'))throw Error('Selecciona un responsable.');
 const status=catalogs().statuses.find(s=>s.name===v('status'));if(!status)throw Error('Selecciona un estado.');
 if(status.group_code==='CERRADA'&&(!v('outcome')||!v('resolution')))throw Error('Registra el resultado y la resolución antes de cerrar.');
 const antecedent=v('antecedentFolio').split(' · ')[0];if(antecedent&&!state.data.tickets.some(t=>t.folio===antecedent))throw Error('Selecciona un ticket antecedente existente.');
 return {title:type?.name||task.title,taskType:type?.name||task.taskType,owner:v('owner'),area:v('area'),priority:v('priority'),status:v('status'),startAt:v('startAt'),dueAt:v('dueAt'),antecedentFolio:antecedent,notes:v('notes'),outcome:v('outcome'),resolution:v('resolution'),checklist:validateChecklist(readChecklist($(prefix+'Checklist')))};
}
function openNewTask(ticketFolio=''){
 const form=$('newTaskForm');form.reset();
 form.querySelector('.filter-fields').innerHTML=field('Buscar ticket principal','<input id="newTaskTicketSearch" placeholder="Folio, cliente, modelo o serie">')+field('Ticket de esta actividad','<select id="newTaskTicket" name="ticketFolio" required></select>')+activityFields('newTask',{ticketFolio});
 renderActivityTicketOptions('',ticketFolio);updateWorkSummary($('newTaskChecklist'));$('newTaskError').textContent='';$('newTaskDialog').showModal();
}
function showTaskDetail(id){
 const task=state.data.tasks.find(x=>x.id===id);if(!task)return;
 openDetail(detailHeader('ACTIVIDAD · '+task.platformId,task.taskType||task.title)+`<section class="detail-section"><button class="text-action" data-action="ticket" data-id="${e(state.data.tickets.find(t=>t.folio===task.ticketFolio)?.id)}">Ticket #${e(task.ticketFolio)}</button>${task.title!==task.taskType?`<details><summary>Descripción histórica</summary><p>${e(task.title)}</p></details>`:''}${progressMarkup(task)}<div class="filter-fields">${activityFields('taskEdit',task)}</div><button class="button primary" data-action="save-task-update" data-id="${e(id)}">Guardar cambios</button></section>`);updateWorkSummary($('taskEditChecklist'));
}

function openCatalog(kind,prefix){
 const labels={clients:'nuevo cliente',service_categories:'nueva categoría',equipment_models:'nuevo modelo',equipment_serials:'nuevo número de serie',activity_types:'nuevo tipo de actividad',activity_statuses:'nuevo estado de actividad'};
 let dialog=$('catalogDialog');if(!dialog){dialog=document.createElement('dialog');dialog.id='catalogDialog';dialog.className='small-dialog';document.body.append(dialog);}
 dialog.innerHTML=`<form id="catalogForm"><div class="dialog-header"><h2>Agregar ${labels[kind]}</h2><button type="button" data-action="close-dialog" class="icon-button" aria-label="Cerrar">×</button></div><div class="dialog-body">${field(kind==='equipment_serials'?'Número de serie':'Nombre','<input name="name" required maxlength="120" autocomplete="off">')}${kind==='equipment_models'?field('Tipo de equipo',`<input name="equipment_type" list="equipmentKinds" required maxlength="120"><datalist id="equipmentKinds">${unique(catalogs().models.map(m=>m.equipment_type)).map(t=>`<option value="${e(t)}"></option>`).join('')}</datalist>`):''}${kind==='activity_statuses'?field('Grupo estadístico obligatorio',selectControl('catalogGroup','group_code',['POR INICIAR','EN CURSO','DETENIDA','CERRADA']))+field('Definición','<textarea name="description" required maxlength="500" rows="2"></textarea>')+'<label><input type="checkbox" name="is_cancelled"> Es una cancelación (solo grupo CERRADA)</label>':''}<p id="catalogError" class="form-error" role="alert"></p></div><div class="dialog-footer"><button class="button primary" type="submit">Guardar en catálogo</button></div></form>`;
 $('catalogForm').onsubmit=async event=>{
  event.preventDefault();const button=event.submitter;button.disabled=true;
  try{
   const values=formPayload(event.target),name=values.name.replace(/\s+/g,' ').trim();let body={name};
   const c=catalogs();let existing;
   if(kind==='clients')existing=c.clients.find(x=>catalogKey(x.name)===catalogKey(name));
   if(kind==='service_categories'){body.business_unit=$(prefix+'businessUnit').value;if(!['SUDMAR','PRETTL'].includes(body.business_unit))throw Error('Selecciona primero SUDMAR o PRETTL.');existing=c.categories.find(x=>x.business_unit===body.business_unit&&catalogKey(x.name)===catalogKey(name));}
   if(kind==='equipment_models'){body.equipment_type=values.equipment_type.trim();existing=c.models.find(x=>catalogKey(x.name)===catalogKey(name));}
   if(kind==='equipment_serials'){const model=c.models.find(x=>catalogKey(x.name)===catalogKey($(prefix+'model').value));if(!model)throw Error('Selecciona primero un modelo del catálogo.');body={model_id:model.id,serial_number:name};existing=c.serials.find(x=>x.model_id===model.id&&catalogKey(x.serial_number)===catalogKey(name));}
   if(kind==='activity_types')existing=c.types.find(x=>catalogKey(x.name)===catalogKey(name));
   if(kind==='activity_statuses'){body={name:name.toUpperCase(),group_code:values.group_code,description:values.description,is_cancelled:new FormData(event.target).has('is_cancelled')};if(!body.group_code)throw Error('Selecciona un grupo estadístico.');if(body.is_cancelled&&body.group_code!=='CERRADA')throw Error('Una cancelación debe pertenecer a CERRADA.');}
   const row=existing||await repository.addCatalog(kind,body);
   state.data.catalogs=await repository.catalogs();
...
document.addEventListener('change',event=>{
 for(const prefix of ['newTicket','ticketEdit','newTask','taskEdit']){
  if(event.target.id===prefix+'owner'){
   const p=state.personnel.find(p=>p.name===event.target.value),areas=p?.operationalAreas||[p?.area||''];
   const old=$(prefix+'area');if(old){const select=document.createElement('select');select.id=old.id;select.name='area';select.innerHTML=areas.map(a=>`<option>${e(a)}</option>`).join('');if(!areas.length)select.innerHTML='<option value="">Sin área</option>';old.replaceWith(select);}
  }
  if(!['newTicket','ticketEdit'].includes(prefix))continue;
  if(event.target.id===prefix+'businessUnit'){$(prefix+'serviceCategory').value='';$(prefix+'serviceCategoryList').innerHTML=catalogs().categories.filter(x=>x.business_unit===event.target.value).map(x=>`<option value="${e(x.name)}"></option>`).join('');}
  if(event.target.id===prefix+'model'){
   const m=catalogs().models.find(x=>catalogKey(x.name)===catalogKey(event.target.value));$(prefix+'equipmentType').value=m?.equipment_type||'';$(prefix+'serial').value='';$(prefix+'serialList').innerHTML=catalogs().serials.filter(x=>x.model_id===m?.id).map(x=>`<option value="${e(x.serial_number)}"></option>`).join('');
  }
 }
});
document.addEventListener('input',event=>{
